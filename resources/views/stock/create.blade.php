...
<div class="mb-3 text-muted small"><span class="text-danger">*</span> Wajib diisi</div>
...
// Fungsi untuk mengambil stok produk berdasarkan lokasi gudang/toko
...
// Variabel global untuk halaman saat ini
let currentPage = 1;
// Fungsi utama merender baris produk dengan pagination dan filter
function renderRows(){
  ...
  const html = pageItems.map((p, i) => {
    ...
    return `<div class="opname-row" id="opRow${gi}" data-original="${stock}"><input type="hidden" name="items[${gi}][product_id]" value="${p.id}"><div class="row g-2 align-items-center"><div class="col-md-5"><strong>${p.name}</strong><div class="small text-muted">${p.code} • ${p.category||'-'}</div></div><div class="col-md-2 text-center"><div class="small text-muted">Stok Sistem</div><strong class="${stock < 0 ? 'diff-minus' : ''}">${stock}</strong><div class="small text-muted">${p.unit}</div></div><div class="col-md-3"><div class="small text-muted mb-1">Stok Aktual</div><input type="number" name="items[${gi}][stock_actual]" class="form-control form-control-sm actual-input" value="${stock}" data-original="${stock}" data-row="${gi}" oninput="calcDiff(this)" onwheel="this.blur()"></div><div class="col-md-2 text-center"><div class="small text-muted">Selisih</div><strong class="diff-zero" id="diff${gi}">0</strong></div></div><div class="mt-2" id="notesRow${gi}" style="display:none"><input type="text" name="items[${gi}][notes]" class="form-control form-control-sm" placeholder="Keterangan perubahan (Wajib diisi)"><small class="text-danger mt-1 d-block">* Keterangan wajib diisi karena ada perubahan fisik</small></div></div>`;
  }).join('');
...
// Fungsi menghitung selisih stok sistem dengan stok aktual
...
// Fungsi memperbarui total produk yang berubah (selisih > 0)
...
// Event listener filter nama produk
document.getElementById('filterInput').addEventListener('input', renderRows);
// Fungsi inisialisasi kategori produk untuk filter dropdown
