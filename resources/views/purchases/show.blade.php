            ...
            <div class="card-body p-4">
                                ...
                                <td class="text-end">
                                    @if(($purchase->status ?? 'approved') === 'draft' && auth()->user()->role === 'pemilik')
                                        <div class="input-group input-group-sm justify-content-end" style="min-width:170px">
                                            ...
                                            <input type="number" name="items[{{ $detail->id }}][unit_buy_price]" class="form-control text-end" min="0" step="any" value="{{ old('items.'.$detail->id.'.unit_buy_price', $detail->unit_buy_price) }}" required>
                                        </div>
                                    @elseif(($purchase->status ?? 'approved') === 'draft' && auth()->user()->role !== 'pemilik')
                ...
                @if(($purchase->status ?? 'approved') === 'draft' && auth()->user()->role === 'pemilik')
                    <div class="d-flex justify-content-end mt-3">
                        <button class="btn btn-primary" onclick="return confirm('Approve draft restok ini? Stok dan HPP akan diperbarui setelah disimpan.')">
