...
@section('content')
<div class="page-hdr"><div class="page-hdr-left"><h1><i class="bi bi-pencil-square me-2" style="color:var(--primary)"></i>Edit Draft Pembelian</h1><nav aria-label="breadcrumb"><ol class="breadcrumb"><li class="breadcrumb-item"><a href="{{ route('purchases.index') }}">Pembelian</a></li><li class="breadcrumb-item"><a href="{{ route('purchases.show', $purchase) }}">{{ $purchase->invoice_number }}</a></li><li class="breadcrumb-item active">Edit</li></ol></nav></div></div>
<form method="POST" action="{{ route('purchases.update', $purchase) }}" id="purchase-form">@csrf @method('PUT')
<div class="row g-3">
    <div class="col-lg-8">
        <div class="card"><div class="card-header"><h6>Informasi Faktur Supplier</h6></div><div class="card-body row g-3">
            <div class="col-md-6 col-xxl-4"><label class="form-label">No. Faktur Supplier <span class="text-danger">*</span></label><input type="text" name="invoice_number" id="invoice_number" class="form-control @error('invoice_number') is-invalid @enderror" value="{{ old('invoice_number', $purchase->invoice_number) }}" placeholder="Sesuai surat jalan/faktur supplier" required>@error('invoice_number')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
            ...
            <div class="col-md-6 col-xxl-3"><label class="form-label">Tempat Penyimpanan <span class="text-danger">*</span></label><select name="warehouse_id" id="warehouse_id" class="form-select" required><option value="">— Pilih Gudang/Toko —</option>@foreach($warehouses as $wh)<option value="{{ $wh->id }}" {{ old('warehouse_id', $purchase->warehouse_id) == $wh->id ? 'selected' : '' }}>{{ $wh->code }}{{ $wh->is_store ? ' (Utama)' : '' }} - {{ $wh->name }}</option>@endforeach</select></div>
            <div class="col-md-6 col-xxl-2"><label class="form-label">Tanggal Beli <span class="text-danger">*</span></label><input type="date" name="purchase_date" id="purchase_date" class="form-control" value="{{ old('purchase_date', $purchase->purchase_date ? \Carbon\Carbon::parse($purchase->purchase_date)->format('Y-m-d') : date('Y-m-d')) }}" required></div>
            <div class="col-12"><label class="form-label">Catatan</label><input type="text" name="notes" id="notes" class="form-control" value="{{ old('notes', $purchase->notes) }}" placeholder="Keterangan tambahan..."></div>
        </div></div>
    ...
    <div class="col-lg-4"><div class="card" style="position:sticky; top:76px"><div class="card-header"><h6>Ringkasan</h6></div><div class="card-body"><div class="alert alert-info"><strong>Mode Edit Draft</strong>: Simpan perubahan daftar pembelian sebelum melakukan approve.</div><div class="d-flex justify-content-between mb-2"><span>Total Item</span><strong id="summaryItems">0 jenis</strong></div><div class="d-flex justify-content-between mb-2"><span>Total Qty</span><strong id="summaryQty">0</strong></div><hr>@if(auth()->user()->role === 'pemilik')<div class="d-flex justify-content-between"><span>Total Pembelian</span><strong class="text-success" id="summaryTotal">Rp 0</strong></div>@endif<button type="submit" class="btn btn-primary w-100 mt-4"><i class="bi bi-check-circle me-2"></i>Simpan Perubahan Draft</button><a href="{{ route('purchases.show', $purchase) }}" class="btn btn-outline-secondary w-100 mt-2">Batal</a></div></div></div>
</div></form>
@endsection
...
$existingDetailsArray = $purchase->details->map(fn($d) => [
    'product_id' => $d->product_id,
    'search' => $d->product->product_name . ' (' . $d->product->product_code . ')',
    'qty' => $d->qty,
    'unit_buy_price' => (int) $d->unit_buy_price
])->values()->toArray();
@endphp
<script>
let rowIndex = 0;
