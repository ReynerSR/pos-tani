...
<!-- Kartu Ringkasan (Pendapatan, Transaksi, Diskon, Rata-rata) -->
...
<script>
// Konfigurasi dan pengaturan data grafik penjualan harian
const dailyData=@json($dailyChart);new Chart(document.getElementById('salesChart'),{type:'line',data:{labels:dailyData.map(d=>d.date),datasets:[{label:'Pendapatan',data:dailyData.map(d=>d.total),borderColor:'#1e8449',backgroundColor:'rgba(30,132,73,.1)',borderWidth:2,fill:true,tension:.35}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{y:{beginAtZero:true,ticks:{callback:v=>'Rp '+Number(v).toLocaleString('id-ID')}},x:{grid:{display:false}}}}});
</script>
<script>
