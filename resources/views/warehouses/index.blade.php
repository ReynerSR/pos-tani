...
<div class="card">
    ...
    <div class="table-wrapper"><table class="table mb-0"><thead><tr><th>#</th><th>Kode</th><th>Nama</th><th>Lokasi</th><th>Aktif?</th><th style="width:100px">Aksi</th></tr></thead><tbody>
        @forelse($warehouses as $i => $w)
        <tr><td class="text-muted small">{{ $warehouses->firstItem()+$i }}</td><td><strong>{{ $w->code }}{{ $w->is_store ? ' (Utama)' : '' }}</strong></td><td>{{ $w->name }}</td><td>{{ $w->location ?? '-' }}</td><td><span class="badge bg-{{ $w->is_active ? 'success' : 'danger' }}">{{ $w->is_active ? 'Aktif' : 'Nonaktif' }}</span></td><td><div class="d-flex gap-1"><a href="{{ route('warehouses.edit',$w) }}" class="btn btn-sm btn-icon btn-outline-primary"><i class="bi bi-pencil"></i></a>@if(auth()->user()->role === 'pemilik')<form method="POST" action="{{ route('warehouses.destroy',$w) }}" onsubmit="return confirm('Hapus lokasi ini? Lokasi hanya bisa dihapus jika tidak memiliki produk/stok.')">@csrf @method('DELETE')<button class="btn btn-sm btn-icon btn-outline-danger"><i class="bi bi-trash"></i></button></form>@endif</div></td></tr>
